...
const handleUpvote = async () => {
	if (!userUpvoted.value) {
		...
		try {
			...
			console.log(joke.value.upvotes);
...
onMounted(async () => {
	try {
		...
		joke.value.upvotes = jokeResponse.data.data.upvotes.map(
			(upvoteObj) => upvoteObj._id
		);
		...
		if (store.token !== "") {
			...
			console.log(favResponse.data.data.favorites);
			console.log(joke.value._id);
			if (
				favResponse.data.data.favorites.some(
					(favJoke) => favJoke._id === joke.value._id
				)
			) {
				userFavorited.value = true;
			}
	...
	>
		...
		>
			<div class="flex flex-wrap items-center">
				<div
					class="h-[32px] w-[32px] rounded-full bg-slate-300 hover:cursor-pointer"
				>
			...
			<div class="mr-20">
				...
				>
					...
					<div
						v-if="toggleDropdown"
						class="origin-top-right absolute mt-2 w-min rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-30"
						role="menu"
