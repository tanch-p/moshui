...
router.get("/jokes/:username", async (req, res) => {
	...
	try {
		const data = await User.find({ username: username }).populate("jokes");
		// if (!user) {
...
router.get("/:username/favorites", authenticateToken, async (req, res) => {
	const { username } = req.params;
	try {
		const data = await User.find({ username: username }).populate("favorites");
		res.status(200).json({
			message: "successfully found all jokes by user",
			data: data[0],
		});
